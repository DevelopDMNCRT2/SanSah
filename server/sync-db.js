...
async function sync() {
  ...
  try {
    ...
    // 6. Items Pedido
    const items = await neonPrisma.itemPedido.findMany();
    console.log(`📝 Importando ${items.length} ítems de pedido...`);
    for (const item of items) {
      await localPrisma.itemPedido.upsert({
        where: { id: item.id },
        update: item,
        create: item
      });
